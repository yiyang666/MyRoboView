...
Json::Value StateStore::snapshot(Clock::time_point now) const {
  std::lock_guard<std::mutex> lock(mutex_);
  Json::Value result;
  result["schema_version"] = 2; result["transport"] = "ros2";
...
std::vector<Json::Value> StateStore::due_frames(Clock::time_point now) {
  ...
  for (const auto &topic : state["topics"]) {
    ...
    if (entry.last_emit != Clock::time_point{} && std::chrono::duration<double>(now - entry.last_emit).count() + 1e-9 < period) continue;
    entry.last_emit = now;
    Json::Value frame;
    frame["schema_version"] = 2; frame["type"] = topic["event"];
    frame["topic_id"] = topic["id"]; frame["topic"] = topic["topic"];
    frame["state"] = topic["state"]; frame["age_sec"] = topic["age_sec"];
    frame["received_count"] = topic["count"]; frame["receive_hz"] = topic["hz"];
    frame["error"] = topic["error"]; frame["data"] = topic["data"];
