...
void validate_config(const Json::Value &cfg) {
  require(cfg["schema_version"].isInt() && cfg["schema_version"].asInt() == 2, "schema_version must be 2");
  ...
  number(nav["broadcast_hz"], 0.1, 1000.0 / server["tick_ms"].asDouble(), "navigation.broadcast_hz");
  ...
  for (const auto &t : cfg["topics"]) {
    ...
    require(events.insert(t["event"].asString()).second && t["event"] != "nav_state" && t["event"] != "hello" && t["event"] != "error", "Duplicate/reserved event");
    number(t["broadcast_hz"], 0.1, 1000.0 / server["tick_ms"].asDouble(), "broadcast_hz");
