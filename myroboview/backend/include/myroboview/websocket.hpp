...
class TelemetrySocket : public drogon::WebSocketController<TelemetrySocket, false> {
 public:
  TelemetrySocket(std::shared_ptr<BroadcastHub> hub, std::shared_ptr<StateStore> store) : hub_(std::move(hub)), store_(std::move(store)) {}
