...
# 终端二：同样加载环境后执行
ros2 run myroboview_backend myroboview_server --config "$PWD/myroboview/backend/config/myroboview.json"
```

分别按 Ctrl+C 关闭。自定义配置也可通过启动脚本的 `ROBOTAPP_CONFIG` 和 `MYROBOVIEW_CONFIG` 环境变量指定。
前端在第三个终端运行 `npm start --prefix myroboview/frontend`，打开 http://127.0.0.1:3000，按 Ctrl+C 关闭。
