...
#include <iostream>
void check(bool ok) { if (!ok) throw std::runtime_error("State machine assertion failed"); }
int main() {
  ...
  check(walk.running_status == State::RUNNING && walk.current_action == "WALK");
  check(turn.current_action == "TURN" && paused.running_status == State::PAUSED);
  check(again.running_status == State::IDLE);
  check(fault.running_status == State::FAULT && fault.motor_status == State::MOTOR_ERROR && fault.current_action == "STOP");
