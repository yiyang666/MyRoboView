...
Json::Value config(const std::string &path) {
    ...
    if (!cfg["robot_id"].isString() || cfg["robot_id"].asString().empty() ||
        cfg["robot_id"].asString().size() > 64)
        throw std::invalid_argument("Invalid robot_id");
    if (cfg["scenario"] != "nominal" && cfg["scenario"] != "fault")
...
class RobotApp : public rclcpp::Node {
 public:
    explicit RobotApp(Json::Value cfg)
        ...
        motors_ = create_publisher<node_app_msgs::msg::MotorHealthArray>(
            name("motor_health"), rclcpp::QoS(10).reliable());
        timers_.push_back(create_wall_timer(period("robot_state"), [this] {
            auto msg = robotapp::state_at(
                elapsed(), cfg_["state_step_sec"].asDouble(), fault());
        ...
        timers_.push_back(create_wall_timer(period("motor_health"), [this] {
            node_app_msgs::msg::MotorHealthArray msg;
            msg.header.stamp = now();
            msg.header.frame_id = "base_link";
            msg.robot_id = cfg_["robot_id"].asString();
            for (unsigned i = 0; i < cfg_["motor_count"].asUInt(); ++i) {
                ...
                m.motor_id = i;
                m.name = "motor_" + std::to_string(i);
                m.online = !(fault() && i == 0);
                m.direction = i % 2 ? m.REVERSE : m.FORWARD;
                m.temperature_celsius =
                    fault() && i == 0 ? 85
                                      : 32 + i + 2 * std::sin(elapsed() / 5);
                m.bus_voltage = 48.0;
                m.position_zero_rad = 0.01 * i;
...
int main(int argc, char **argv) {
    try {
        // 默认配置在安装前缀 etc/robotapp/ 下（布局与原项目对齐），--config
        // 可覆盖
        ...
        std::cout << "robotapp: publishing RobotState, Imu, MotorHealthArray"
                  << std::endl;
        rclcpp::spin(node);
        rclcpp::shutdown();
        return 0;
