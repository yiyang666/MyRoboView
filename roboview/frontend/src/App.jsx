...
import { enumLabel } from './constants/robotEnums';
...
// 指标展示：枚举字段（current_mode/health_status 等）走中文映射，其余走数值格式化
function displayMetric(data, metric) {
  const raw = field(data, metric.field);
  return enumLabel(metric.field, raw) ?? display(raw, metric.scale);
}
    ...
    <main className="main-content">
      ...
      {menu === 'navigation' ? <div className="navigation-section"><MapNavigation navState={navState} connected={connected} /></div> : menu === 'motors' ? <MotorStatus snapshot={snapshot} connected={connected} /> : menu === 'system' ? <SystemInfo robot={snapshot?.robot} connected={connected} /> : <div className="page-container">
        ...
        {menu === 'telemetry' ? <div className="sensor-grid">{topics.map(topic => <article key={topic.id} className={`sensor-card ${!connected || topic.state !== 'live' ? 'disconnected' : ''}`}>
          ...
          <div className="sensor-values">{topic.metrics.map(metric => <div className="sensor-value" key={metric.field}><label>{metric.label}</label><span>{displayMetric(topic.data, metric)} <small>{metric.unit}</small></span></div>)}</div>
