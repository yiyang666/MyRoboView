...
// 模式/动作/运行状态为消息枚举值，需经中文映射显示（见 constants/robotEnums.js）
import { CURRENT_MODE, CURRENT_ACTION, RUNNING_STATUS } from '../constants/robotEnums';
...
export default function StatusBar({ snapshot, connected }) {
  ...
  const mode = connected && status ? CURRENT_MODE[status.current_mode] : null;
  ...
  return <div className="status-bar">
    <div className="status-left">
      ...
      <div className="status-item"><span className="status-label">状态:</span><span className="status-value">{!connected ? '服务离线' : !status ? '等待状态' : `${RUNNING_STATUS[status.running_status] || '未知'} · ${CURRENT_ACTION[status.current_action] || '—'}`}</span></div>
    </div>
