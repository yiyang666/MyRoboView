...
export default function MotorStatus({ snapshot, connected }) {
  ...
  return <div className="page-container">
    ...
    <div className="motor-summary"><span>电机：{motors.length} 个 · {fresh ? `在线 ${motors.filter(m => m.online).length} 个` : '数据未更新，历史值仅供参考'}</span>
      <label>名称映射 <select value={profile} onChange={event => setProfile(event.target.value)}><option value="product">{URDF_CONFIG.displayName} 关节</option><option value="generic">消息原始名称</option></select></label>
    </div>
